...
export class PhotosSelector extends Component {
    ...
    return (
      <View style={[localStyles.outerContainer, this.props.style]}>
        <ListView
          ...
          onLayout={this._storeDimensions}
        />
        {this._getTabBar()}
      </View>
    );
  ...
  _getRow(data, sectionId, rowIndex) {
    if (
      ...
      this.state.selectedTab == TAB_RECENT &&
      !this.isFetching &&
      !this.fetchedAllPhotos
    ) {
      this._getCameraRollAssets();
    }
    ...
    return (
      <View
        key={ROW_PREFIX + rowIndex}
        style={[localStyles.rowContainer, {height: height}]}>
  ...
  _getRowElements(data, rowNumber) {
    ...
    for (var i = 0; i < this.props.columns; i++) {
      toReturn.push(
        <View
          key={COLUMN_PREFIX + this.state.selectedTab + i}
          style={localStyles.rowElement}>
  ...
  _getImageIndex(data, row, column) {
    ...
    if (data) {
      ...
      return (
        <TouchableOpacity
          style={[localStyles.touchElement]}
          activeOpacity={0.5}
          focusedOpacity={0.5}
          onPress={this._getElementClick(row, column, data)}>
          <ImageBackground
            style={localStyles.fetchedImageStyle}
            source={thumbnailSrc}
            ref={this._storeRef(row, column)}
            resizeMethod={'resize'}
            onError={(error) => {
              console.log('[PhotoSelector] load image error: ' + error);
            }}>
            {renderIf(
              data.type == PSConstants.PS_TYPE_360_PHOTO,
              <Image
                style={{marginRight: 10}}
                source={require('../res/icon_360_photo.png')}
              />,
            )}
            {renderIf(
              data.type == PSConstants.PS_TYPE_VIDEO,
              <Image
                style={{marginRight: 10}}
                source={require('../res/icon_video.png')}
              />,
            )}
            {renderIf(
              data.type == PSConstants.PS_TYPE_360_VIDEO,
              <Image
                style={{marginRight: 10}}
                source={require('../res/icon_360_video.png')}
              />,
            )}
          </ImageBackground>
        </TouchableOpacity>
  ...
  _updatePhotoSelection(row, column) {
    let previousSelection =
      this.refList[this._getRefKey(this.selectedRow, this.selectedColumn)];
    ...
    if (newSelection) {
      newSelection.setNativeProps({style: {opacity: 0.5}});
    }
  ...
  _getTabBar() {
    return (
      <View style={localStyles.tabBarContainer}>
        <TouchableOpacity
          style={localStyles.tabTouch}
          activeOpacity={0.5}
          onPress={this._getTabPress(TAB_STOCK)}>
          <Text
            style={[
              localStyles.tabBarText,
              {
                fontWeight:
                  this.state.selectedTab == TAB_STOCK ? 'bold' : 'normal',
              },
            ]}>
            Stock
          </Text>
        ...
        <TouchableOpacity
          style={localStyles.tabTouch}
          activeOpacity={0.5}
          onPress={this._getTabPress(TAB_RECENT)}>
          <Text
            style={[
              localStyles.tabBarText,
              {
                fontWeight:
                  this.state.selectedTab == TAB_RECENT ? 'bold' : 'normal',
              },
            ]}>
            Recent
          </Text>
  ...
  _getCameraRollAssets() {
    ...
    CameraRoll.getPhotos({
      ...
      .then((retValue) => {
        ...
        if (
          this.fetchCount < this.props.searchQuota &&
          retValue.page_info.has_next_page
        ) {
          this._updateDataSource(() => {
            this._getCameraRollAssets();
          });
      ...
      .catch((err) => {
        console.log(
          '[PhotoSelector] error while fetching Camera Roll assets: ' +
            err.message,
        );
      });
  }
  ...
  _filterPhotos(edges, photos360, photos) {
    for (var i = 0; i < edges.length; i++) {
      ...
      let image = {
        ...
        type: this._is360Photo(edge.node.image)
          ? PSConstants.PS_TYPE_360_PHOTO
          : PSConstants.PS_TYPE_PHOTO,
      };
      if (this._isVideo(image.source.uri)) {
        image.type = this._is360Photo(edge.node.image)
          ? PSConstants.PS_TYPE_360_VIDEO
          : PSConstants.PS_TYPE_VIDEO;
      }
      photos.push(image);
    }
  }
