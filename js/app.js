...
export class App extends Component {
  constructor(props) {
    ...
    this._onContextMenuRemoveButtonPressed =
      this._onContextMenuRemoveButtonPressed.bind(this);
  ...
  _renderContextMenu() {
    let selectedItemIndex = this.props.currentItemSelectionIndex;
    let clickState = this.props.currentItemClickState;
    let totalHeight = 120;
  ...
  _onPhotoSelected(index, source) {
    this.props.dispatchChangePortalPhoto(
      this.state.lastSelectedPortalUUID,
      source,
    );
  }
        ...
        <View style={localStyles.shareScreenContainerTransparent}>
          ...
          {renderIf(
            ...
            <Video
              ...
              repeat={false}
              style={localStyles.backgroundVideo}
              onEnd={() => {
          ...
          {renderIf(
            ...
            <View
              ...
              <TouchableOpacity
                ...
                style={localStyles.previewPlayButtonContainer}
                underlayColor="#00000000">
                <Image
                  source={require('./res/btn_play.png')}
                  style={localStyles.previewPlayButton}
                />
              </TouchableOpacity>
            </View>,
          )}
          ...
          <View
            ...
            <ShareScreenButton
              onPress={() => {
                this.props.dispatchDisplayUIScreen(
                  UIConstants.SHOW_MAIN_SCREEN,
                );
              }}
              buttonState={'off'}
              stateImageArray={[
                require('./res/btn_close.png'),
                require('./res/btn_close.png'),
              ]}
          ...
          <View
            ...
            <ShareScreenButton
              onPress={() => {
                this._saveToCameraRoll();
              }}
              buttonState={this.state.haveSavedMedia ? 'on' : 'off'}
              stateImageArray={[
                require('./res/btn_saved.png'),
                require('./res/btn_save.png'),
              ]}
  ...
  _renderRecord() {
    ...
    if (this.props.currentScreen === UIConstants.SHOW_RECORDING_SCREEN) {
      recordViews.push(
        <View
          ...
          <Text style={localStyles.recordingTimeText}>
    ...
    if (
      ...
      recordViews.push(
        <View
          key="record_two_button_container"
          style={{
            position: 'absolute',
            flex: 1,
            flexDirection: 'row',
          ...
          <RecordButton
            ...
            stateImageArray={[
              require('./res/btn_stop.png'),
              require('./res/btn_record.png'),
            ]}
  ...
  _onAdd() {
    this.setState({addingModal: !this.state.addingModal});
  }
  ...
  _startRecording() {
    ...
    this._arNavigator._startVideoRecording(
      'figment_video',
      false,
      (errorCode) => {
        this._displayVideoRecordAlert(
    ...
    this._arNavigator._stopVideoRecording().then((retDict) => {
      if (!retDict.success) {
        if (retDict.errorCode === ViroConstants.RECORD_ERROR_NO_PERMISSION) {
          this._displayVideoRecordAlert(
            'Recording Error',
            'Please allow camera record permissions!',
          );
        }
      }
      this.setState({
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    dispatchToggleEffectSelection: (index) =>
      dispatch(toggleEffectSelection(index)),
    dispatchChangeModelLoadState: (index, loadState) =>
      dispatch(changeModelLoadState(index, loadState)),
    dispatchChangePortalLoadState: (index, loadState) =>
      dispatch(changePortalLoadState(index, loadState)),
    dispatchDisplayUIScreen: (uiScreenState) =>
      dispatch(displayUIScreen(uiScreenState)),
    dispatchChangePortalPhoto: (index, source) =>
      dispatch(changePortalPhoto(index, source)),
    dispatchChangeItemClickState: (index, clickState, itemType) =>
      dispatch(changeItemClickState(index, clickState, itemType)),
  };
};
