...
export class figment extends Component {
  ...
  _renderModels(modelItems, startingBitMask) {
    ...
    if (modelItems) {
      ...
      Object.keys(modelItems).forEach(function (currentKey) {
        if (
          modelItems[currentKey] != null &&
          modelItems[currentKey] != undefined
        ) {
  ...
  _onLoadCallback(uuid, loadState) {
    this.props.arSceneNavigator.viroAppProps.loadingObjectCallback(
      uuid,
      loadState,
    );
  }
...
const mapDispatchToProps = (dispatch) => {
  return {
    dispatchARTrackingInitialized: (trackingNormal) =>
